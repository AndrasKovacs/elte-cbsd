    ...
    respond hCenterIn $ \(msg :: CenterLogic state move) -> do
      case msg of
        ...
        (CL_GET_START_STATE :: CenterLogic state move) -> do
          ...
          printf "LOGIC: sending GET_START_STATE: %s\n" (show $ encode resp)
          pure $ Just $ resp
        ...
        CL_EVALUATE_MOVE (ReqEvaluateMove strec move) -> do
          ...
          let resp = LC_EVALUATE_MOVE $ ResEvaluateMove $
               State id status state' (switch player) :: LogicCenter state move

          printf "LOGIC: sending EVALUATE_MOVE: %s\n"
            (show $ encode resp)                       
          pure $ Just resp
        
        CL_POSSIBLE_MOVES (ReqPossibleMoves (State _ _ state player)) -> do
          printf "LOGIC: received POSSIBLE_MOVES: %s\n" (show $ encode msg)
          let resp = LC_POSSIBLE_MOVES $ ResPossibleMoves (moves player state)
          printf "LOGIC: sending POSSIBLE_MOVES: %s\n" (show $ encode resp)          
          pure $ Just resp
